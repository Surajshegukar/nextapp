import { Podcast, PodcastType, getPodcasts } from "@/app/functions/getPodcasts";
...
export async function generateMetadata({
  ...
  const podcastData = podcast?.data?.find(
    (podcast: Podcast) => slugify(podcast.podcast_name) === params.title
  );
  ...
  return {
    title: `${podcastData.podcast_name} | Fyrre Magazine`,
  };
}
      ...
      <main className="max-w-[95rem] w-full mx-auto px-4 pb-12 sm:pt-4 xs:pt-2 md:pb-4 sm:pb-2 xs:pb-2">
        ...
        <article className="max-w-[75rem] w-full mx-auto flex flex-wrap gap-24">
          <article className="flex flex-col lg:w-1/4">
            <img
              className=""
              src={`http://localhost:3000/uploads/images/${podcastData.image}`}
              alt={podcastData.podcast_name}
            />
            ...
            <div className="flex flex-col gap-4 pt-8">
              <div className="flex flex-wrap justify-between">
                ...
                <time dateTime={podcastData.publish_date}>
          ...
          <article className="flex flex-col flex-1 w-full">
            <p className="uppercase font-semibold">Episode {podcastData.id}</p>
            <h1 className="podcast-title">{podcastData.podcast_name}</h1>
            <div
